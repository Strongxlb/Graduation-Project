...
| 环节 | 可写入 thesis 的要点 |
| --- | --- |
| **观测误差模型** | grab sample DPD 可设 σ ≈ 0.02 mg/L（仪器规格）+ **系统偏差项**（方法选择） |
...
1. **T5 似然函数**：观测模型 C_obs ~ Normal(C_true, σ²)，**σ = 0.02 mg/L**（Hach 规格）— Methodology 直接写入参数表。
...
5. **浊度协变量**：数字 DPD 与浊度显著相关 — 若有浊度数据，Methodology 可写 **heteroscedastic** σ(turbidity)。
