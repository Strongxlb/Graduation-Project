...
**为什么是 P1**：提供 **11 段管网现场 k_w**（54 次）+ **实验室管段 k_w** 的对照方法；给出 **管材分级**（无衬铸铁 vs PVC/MDPE/水泥衬）及 k_w 与 **C₀、流速** 的关系。直接支撑 Net1/EPANET 中 `wall_coeff` 取值范围、管材差异和「高反应性管壁受传质限制」的论述 — 与 A1 Rossman 案例 k_w ∈ [0.15, 0.45] **m/day** 可对照（注意单位体系不同，见 §7）。
...
5. **C₀**：低反应性管（尤其 PVC 现场）k_w 与 C₀ **反相关**；实验室 CICL/MDPE/PVC 用幂函数拟合。  
...
| k_w 是否随 C₀、流速变 | 分管材给出经验关系（有限 C₀ / v 范围） |
...
**与现场差异**：多为新管、更高 C₀、更长试验时间、平均 **19±1 °C**、搅拌近似流动。
...
- **低反应性（塑料/水泥衬）**：k_w 低；流速关系 **R² = 0.26** → **管材反应** 为限速。  
- 作者**否定**「高 C₀ 因浓度差减小传质」的简单解释（高 C₀ 应增大传质驱动力，却观测到 k_w 下降）。

### 5.3 k_w 与初始氯浓度 `[原文]` Figs. 7–8、Table 2

**现场 PVC**（C₀ 约 0.15–0.3 mg/L）：

    k_w = 0.26 - 0.73 · C₀     (99% 显著)

→ 外推 k_w = 0 在 C₀ > 0.36 mg/L（作者强调**仅在线性区间内**有代表性）。

**实验室**（C₀ 0.16–1.95 mg/L），幂函数（Table 2）：
...
→ **C₀ 越高，表观 k_w 越低**（低反应性管存在「管壁摄取速率上限」）。无衬铸铁现场**未**见显著 C₀ 关系。

### 5.4 流速与 Reynolds `[原文]` Fig. 9

- **CI、SI**：k_w vs 流速 0.2–0.45 m/s 近似线性（99% 显著）；低于 0.15 m/s 外推为零不现实。  
- **Re**：1.7×10⁴ – 1.7×10⁵（过渡流）；Re 与 k_w 关系与流速类似，低反应性管拟合差。
...
4. 低反应管 k_w 与 C₀ **反相关**（现场 PVC 线性段；实验室幂律）。  
...
4. **Methodology — in situ 思路**：demand monitor（C₀、C、旅行时间）估 k_w = k - k_b — 即使无 Leicester 数据，合成场景可 **模仿该实验设计**。
5. **T5 不确定性**：旅行时间 ±5–15%、氯 ±0.01 mg/L 时 k_w 不确定 — Discussion 引用为 **wall 参数后验应宽** 的文献依据。
6. **低浓度敏感**：PVC 段 C₀ – k_w 关系 — 解释管网末端接近 **0.2 mg/L** 时 wall 校准更不稳定。
...
2. 若测量误差 ±0.01 mg/L，当 C₀ - C 很小时，k_w = k - k_b 是否被 **bulk 扣除** 放大？  
...
| 3 | 仅「wall coefficient、管材」 | 补充 **in situ 方法、C₀ / 流速 / 生物膜、传质 vs 反应限制** |
