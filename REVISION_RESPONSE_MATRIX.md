...
| **Note on the length figure** | The comment's "30-page limit" is not the CIVE70058 rule. The module descriptor sets **12 000 words maximum**, journal-paper style, 12 pt, 1.5 line spacing, 25 mm margins, normally no contents page, Harvard references, and every figure and table numbered, captioned, referenced in the text and source-acknowledged. So length **is** a hard constraint, just measured in words: the 20 Jul draft is already **11 624 words** while covering only the pre-review scope (Steps 1–6 in current terms). Reporting Steps 0 and 7–15 as well means the selection problem is severe — most of this log has to become supporting material rather than main text. |
...
|---|---|---|
| **Warm-up convergence test** (`step0_warmup_convergence.py`) | The 24 h warm-up was never justified, while the high-risk junctions have mean water ages of 34–45 h. | Chlorine concentration criteria pass at **120 h**, chosen as a **pragmatic finite-horizon warm-up**; residual ~5.1% cycle-to-cycle deficit drift remains and water age is **horizon-dependent and unconvergeable** inside the 168 h ceiling. Absolute ages must never be quoted as steady state; the cache was rebuilt and risk severity / top nodes moved. |
...
| **Chlorine concentration unit, 1000× (`step15_unit_equivalence.py`)** | Self-audit after the revision was otherwise complete. WNTR stores concentration in kg/m³ whatever `inpfile_units` says, so `initial_quality = 1.0` simulated **1000 mg/L**; the source, the tanks, σ and the 0.2 mg/L threshold were all 1000× too high for the whole life of the project. | Invisible by construction: first-order kinetics are linear in C and σ and the threshold sat on the same scale, so every ratio was preserved and no check could fail. The reaction coefficients were never affected (`k_b` is 1/time, `k_w` is length/time), which is why the analytic known-answer test passed throughout. The factor had in fact been measured already — the inherited notebook records `×1000` for a **zero-order** bulk coefficient — but was diagnosed as a zero-order quirk rather than as the concentration unit, so it was never applied to concentrations. Fixed at the model boundary. Separately and deliberately, the corrected configuration adopts a **stricter solver tolerance of 1e-5 mg/L** against EPANET's 0.01 mg/L default. This is **not** a unit conversion — WNTR writes `TOLERANCE` to the `.inp` verbatim while converting concentrations, so EPANET reads it in mg/L (asserted in `step13` arm 5 by reading the written file back). Leaving the default while correcting the concentrations would have coarsened the solver 1000×; the strict value holds the tolerance/source ratio of the superseded run fixed so the comparison isolates the units. **Measured effect, now tracked in `step15_full_regression.json` (`step15b_full_regression.py`): the reported field is unchanged to 2.3 × 10⁻⁷ across all 8192 candidates; of 4915 numeric fields across 26 artifacts, 34 moved by more than 1e-4 relative and none of them is an estimate, interval, ranking or risk number; of the log's 3582 numbers, three moved — an analytic value's sixth decimal, that same value's own relative error, and an optimiser iteration count. The first attempt at this comparison was made with throwaway scripts and got two of its three claims wrong, which is why it now has a step of its own.** Every scientific conclusion stands verbatim; what is corrected is the physical interpretation, on which every sensor-accuracy, threshold and dosing statement depends. |
| **Demand read without its pattern (`step10`, `step12`)** | The consequence axis of the risk register, and every demand-weighted number, came from WNTR's `base_demand` field. Four Net3 junctions (15, 35, 123, 203) encode a large demand as 1 GPM multiplied by a large pattern, so the base field alone reports them as 0.063 L/s each instead of 16.7 / 108.4 / 75.3 / 284.6. | Switched to `wntr.metrics.hydraulic.average_expected_demand`, which applies the pattern and the global multiplier. The network total goes from 192.6 to **690.7 L/s**; the consequence terciles from 1.16 / 3.39 to **1.67 / 4.38 L/s**; scenario demand at risk from 36.3 / 45.0 / 47.8 / 49.4 to **55.4 / 64.7 / 67.8 / 69.4 L/s** — but as a *share* of network demand from 18.8–25.6 % down to **8.0–10.0 %**, because the denominator grew faster than the numerator. The high/very-high count in scenarios C and D falls 14/15 → **13/13**. Unlike the unit error this one is **not** a relabelling: it changes which nodes sit in which consequence band and it strengthens Step 10's central finding — demand-weighted `E[D]` drops from 2.36 to **1.23 h**, i.e. risk sits at small consumers even more sharply than first reported. No conclusion reverses. |
