...
15. **The chlorine concentration unit was wrong by 1000×; corrected, and measured to change nothing.** WNTR stores concentration internally in kg/m³ whatever `inpfile_units` says, so `initial_quality = 1.0` simulated **1000 mg/L** — source, tanks, σ and the 0.2 mg/L threshold all 1000× too high. It was invisible because first-order kinetics are linear in C and σ and the threshold sat on the same scale, so every ratio was preserved; the reaction coefficients carry no mass unit and were never affected. Fixed at the model boundary. The corrected configuration **also adopts a stricter solver tolerance, 1e-5 mg/L against EPANET's 0.01 default** — a separate, deliberate numerical choice, not a unit conversion: WNTR writes `TOLERANCE` into the `.inp` verbatim (unlike concentrations, which it converts), so leaving the default in place while correcting the concentrations would have coarsened the solver 1000× and introduced a **17.8%** maximum relative error. The strict value holds the tolerance/source ratio of the superseded run fixed, which is what lets the comparison isolate the units. Measured on the full 8192-candidate library and tracked in `step15_full_regression.json`: reported field unchanged to **2.3 × 10⁻⁷**, and three of the log's 3582 numbers moved (an analytic value's sixth decimal, that value's own relative error, and an optimiser iteration count). Every scientific conclusion stands verbatim; what is corrected is the physical interpretation on which the sensor-accuracy, threshold and dosing statements depend (Step 15).
...
17. **A breach-probability risk register and a severity register are different products, and the ageing stress shows only on the second.** Scoring expected below-threshold duration `E[D]` on pre-declared absolute hours against the same consequence axis, the count of *persistent* consumers (below 0.2 mg/L for at least half the 48 h window) runs **1 → 4 → 6 → 8** across the four scenarios, while the breach count runs 10 → 12 → 13 → **13** and saturates between heatwave and heatwave+ageing. A register built on breach probability alone would report that ageing adds almost nothing. Neither axis is the correct one, so the register acts on the **higher** of the two bands — a declared rule, not a preference. Two limits are measured and stated: the ordering of the two banded scores is **not** implied by `E[D] ≤ T·P_min` (the nearest junction is 0.939 h from inverting), and a band-change escalation test is blind where the matrix saturates or caps — node 243 spends the **entire window** below the threshold under the ageing scenario and is flagged by neither test, because a minor consumer tops out at `medium` by construction (Step 12).
18. **Methodological lesson:** single-realisation results were overturned twice under scrutiny and the informal score reverses conclusions in **three** places (threshold, structural bias, bias curvature); a fourth effect is an understated magnitude, not a reversal. A separate failure mode is method-independent: **a summary sentence that does not follow from the table above it** (Step 8b). Defences that worked were paired controls, ensembles over arbitrary choices, a formal likelihood, multi-route corroboration, and reading each conclusion against its own table.
