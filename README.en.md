...
12. **Scenario headlines (formal ensemble):** baseline 21 nodes / 36.3 L/s at risk; heatwave 29 / 47.8; heat+ageing 31 / 49.4. Tested +30% source dose improves continuous severity but does not restore baseline demand-at-risk (Step 12).
13. **`k_b` ±20% changes the operational shortlist — this conclusion is reversed from the earlier record.** Formal weighting, 30 noise realisations, median risk field: the full-network Spearman stays high (0.976 / 0.933), but the **top-6 set retains only four of its six reference hot-spots at both ±20% (Jaccard 0.50)** — Jaccard compares set membership, not order within the set. The two indices agree rather than conflict: Spearman is dominated by the 86 low/mid-risk junctions, while the top-6 turns over as soon as nodes swap across the 6th/7th boundary. **The sensitivity appears under both weightings (informal also 0.50), so it is not a weighting artefact**; what the formal rule changes is the parameter displacement (1.4–1.6 SD against 0.28–0.40 SD). The superseded claim was a reporting inconsistency: "top nodes unchanged" was written directly beneath a table of three visibly different node sets. A depth check rules out a cut-off artefact: Jaccard is **worse at k = 3 (0.20)** than at k = 6, the reference 3rd and 4th nodes fall to 8th and **20th** at `k_b = −0.4`, and a node ranked **12th rises to 2nd** at `−0.6`; only the saturated worst node (131, risk 1.0000) is common to every leading set. The median rank change over all 92 junctions is 1.0 place, which is why Spearman stays high while the shortlist turns over. A high whole-network rank correlation is not evidence that the actionable shortlist is invariant (Step 8b).
