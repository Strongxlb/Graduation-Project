...
13. **`k_b` ±20% 会换掉热点，这条结论已被推翻重写。** 正式加权、30 次噪声、按中位风险场排序：全网 Spearman 仍高（0.976 / 0.933），但 **top-6 热点集合在两侧都换掉 2 个（Jaccard 0.50）**；参数上 average/new 被推 **1.4–1.6 个后验标准差**，old 只有 0.56–0.66——弱信息的系数吸收了 bulk 衰减误差，与 Fisher Case B 的膨胀次序（old 1.19×、avg/new 1.84–1.85×）一致。informal 评分把同样的位移报成 0.11–0.40 SD，低估四到五倍，这是「风险图对 `k_b` 不敏感」这个旧结论的来源（Step 8b）。
14. **数值层面的两处修正。** 连续 profile 相对 21 点网格把 95% 半宽外扩 **15–39%**（censored 主口径）；Fisher 原始条件数 215 在先验标准化后只有 **3.2**（Step 7 / 7b）。
15. **方法论教训。** 单实现结果在审查中被推翻过两次，informal 评分在四处反转结论；有效防御是配对控制、对任意选择做 ensemble、用正式似然、以及多路径互相印证。
