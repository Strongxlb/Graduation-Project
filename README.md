...
13. **`k_b` ±20% 会换掉运营优先名单，这条结论已被推翻重写。** 正式加权、30 次噪声、按中位风险场排序：全网 Spearman 仍高（0.976 / 0.933），但 **top-6 只保留了 6 个参考热点中的 4 个、换掉 2 个（Jaccard 0.50）**——Jaccard 比的是**集合成员**而非集合内次序。两个指标不矛盾：Spearman 由 86 个中低风险节点主导，而 top-6 只要第 6/7 名附近互换就会掉。**这个敏感性在 formal 与 informal 下都出现（informal 同样是 0.50），所以不是加权口径造成的**；formal 改变的是**参数**位移幅度（1.4–1.6 SD 对 0.28–0.40 SD）。旧结论的真正病因是**文字与自己的表格矛盾**——它在列出三组明显不同的节点集之后写了「top nodes unchanged」。**深度检查证明这不是 cut-off 假象**：在 k=3 时 Jaccard 反而更差（**0.20**，三个里只共享 1 个），与边界效应的预期相反；`k_b=−0.4` 时参考第 3、第 4 名跌到第 8、**第 20** 名，`−0.6` 时原第 12 名**升到第 2**。唯一在所有情形都留在前列的是 node 131，而它已饱和（全窗口低于阈值，风险 = 1.0000）无法移动。全网 92 节点的排名变化中位数只有 **1 位**——这正是 Spearman 高而名单翻动的原因。教训：全网 Spearman 高，不等于可执行的热点名单稳定（Step 8b）。
