...
> 历史执行计划（**已废弃**，描述的是 Bristol 三 DMA 真实数据方案，与本研究不同）：[`plan1.md`](./plan1.md) ｜ 文献清单：[`background/Literature/literature.md`](./background/Literature/literature.md)
...
- **Step 0 预热选择**：需求模式与泵排班都是 24 h 周期，正确判据是**逐日周期场重现**（容差事先写死）。120 h 时余氯浓度场达到预设容差，但累计缺口仍有约 **5.1%** 的周期漂移、水龄 p95 仍差 **12.8 h**——因此 120 h 是有限时域的务实选择，不是「完全周期稳态」。硬约束：泵 10 的绝对时刻控制只列到 159 h，超过 168 h 泵会永久停转。
...
4. **预热 24 h 不够；120 h 是有限时域务实选择，不是完全收敛。** 余氯浓度场在 **120 h** 达到预设容差，但累计缺口仍有约 **5.1%** 周期漂移；水龄周期差在 120–144 h 仍有 12.8 h，外推约 **600 h**，远超模型 168 h 上限。绝对水龄绝不能当稳态值引用（Step 0）。
...
10. **风险图的物理佐证与三种网络均值。** 风险与 water age 的描述性 Spearman ρ = 0.73（water age 由**同一个**水力模型产生，所以这是**反应无关的内部水力佐证**，不是独立验证）（**空间 block bootstrap 95% `[0.455, 0.897]`**，10 个 k-means 空间块、2000 次重抽；这是保守的描述性宽度，**不是显著性区间**）；普通 p-value 与 iid 节点 bootstrap 都已删除，因为 92 个 junction 共享管道与流路。三种网络均值往相反方向走——仅消费节点更差、需求加权更好——说明风险集中于小用户（Step 10）。
11. **预测成功不能证明参数可辨识。** 留一监测点看起来很好；但去掉某区两个监测点后，该区系数退回先验中点（old → −0.850，SD 保留 100%），而预测几乎不变。空间预测和参数辨识是两个独立主张（Step 11a–d）。
12. **风险结论对加权口径稳健；对预热则只能说「主要格局与情景方向未反转」。** 累计缺口在 120 h 仍有约 5.1% 的周期漂移、水龄在模型 168 h 上限内根本无法收敛，所以**绝对**严重度与绝对水龄仍依赖评估窗口，不能称为对预热稳健。 12 °C baseline 21 nodes / 55.4 L/s at risk；heatwave 29 / 67.8；heat + ageing 31 / 69.4。测试范围内 +30% source dose 改善连续严重度，但未恢复 baseline demand-at-risk（Step 10 / 12）。
...
15. **浓度单位曾错 1000 倍，已修正并实测验证不影响任何结论。** WNTR 内部以 kg/m³ 存储浓度，`inpfile_units = "mg/L"` 只管 `.inp` 读写，所以 `initial_quality = 1.0` 实际模拟的是 **1000 mg/L**——进水、水箱、σ、0.2 mg/L 阈值全部高 1000 倍。**为什么一直没暴露**：一阶动力学对浓度线性，且 σ 与阈值同倍错，所有比值精确不变；反应系数不含质量量纲（`k_b` 是 1/time、`k_w` 是 length/time）因而从未受影响，解析已知答案检验一直通过。修正在模型边界完成。**同时主动收紧了求解器容差：1e−5 mg/L，比 EPANET 默认的 0.01 mg/L 严格 1000 倍** —— 注意这是**另一件事，不是单位换算**：WNTR 把 `TOLERANCE` **原样**写进 `.inp`（浓度则会转换），EPANET 按文件声明的 mg/L 读取。若只改浓度、容差留在默认值，求解器分辨率会被无声粗化 1000 倍，引入最大 **17.8%** 相对误差。选 1e−5 是为了**保持与旧运行相同的容差/源浓度比值**，这样对比才只反映单位这一项。**实测**（全量 8192 组，产物 `step15_full_regression.json`）：报告场相对差 **2.3×10⁻⁷**，日志 3582 个数字中有 3 个变化（解析检验值的第 6 位小数、该值自身的相对误差、优化器迭代计数）。**所有科学结论逐字成立；修正的是浓度的物理解释**——传感器精度、运营阈值、加氯剂量这些工程结论此前建立在错误物理量上，现在才真正成立（Step 15）。
16. **需求读取时漏掉了 pattern——这一处不是「换标签」，是真的改数。** 风险登记表的后果轴原本用 WNTR 的 `base_demand` 字段，但 Net3 有四个 junction 把大需求写成 1 GPM × 一个很大的 pattern，只读 base 会把它们各算成 **0.063 L/s**，而真实值是 16.7 / 108.4 / 75.3 / 284.6——**全网 70% 的水量，被当成四个最小的用户**。改用 `average_expected_demand`（按 pattern 和全局倍数折算）后：全网总需求 192.6 → **690.7 L/s**，后果三分位 1.16 / 3.39 → **1.67 / 4.38 L/s**，各情景 demand at risk 36.3 / 45.0 / 47.8 / 49.4 → **55.4 / 64.7 / 67.8 / 69.4 L/s**；但占全网需求的**比例**反而从 18.8–25.6% 降到 **8.0–10.0%**，因为分母涨得比分子快。它**加强**了第 10 条结论：需求加权 `E[D]` 从 2.36 h 降到 **1.23 h**，风险集中于小用户比原先报告的还要明显。没有任何结论反转（Step 10 / 12）。
17. **breach 登记表和 severity 登记表是两个不同的产品，而老化胁迫只在后者上显形。** 把预期低于阈值时长 `E[D]` 按**预先声明的绝对小时数**分档、乘同一个后果轴：*persistent* 消费节点（48 h 窗口内至少一半时间低于 0.2 mg/L）在四个情景下是 **1 → 4 → 6 → 8**，而 breach 计数是 10 → 12 → 13 → **13**，在 heatwave → heatwave+ageing 之间**饱和**。只用 breach 概率的登记表会得出「老化几乎没有额外影响」这个错误结论。两个轴都不是正确的那个，所以登记表按**两者较高的一档**行动——这是**声明的规则**，不是偏好。两条限制已实测并写明：两个分档分数的次序**并非** `E[D] ≤ T·P_min` 所蕴含（最近的节点离反转只差 0.939 h）；band 变化的升级判定在矩阵饱和或封顶处失灵 —— node 243 在老化情景下**整整 48 小时全程**低于阈值，两种判定都不报警，因为 minor 用户在构造上最高只能到 `medium`（Step 12）。
18. **方法论教训。** 单实现结果在审查中被推翻过两次，informal 评分在**三处**反转结论（阈值、结构误差、偏置曲率），第四处只是低估幅度而非反转。还有一类错误与方法无关：**总结句不忠实于自己上方的表格**（Step 8b）。有效防御是配对控制、对任意选择做 ensemble、用正式似然、多路径互证，以及逐行核对文字与表格。
...
- **Discussion**：informal GLUE 的统计低效（有 CRLB 对照的定量实例）、可辨识性梯度、precise-but-biased 只在异质性**有结构**时出现、预测成功与参数辨识必须分开、`k_b` 误设会换掉热点、所需传感器精度对水安全计划的意义、局限（AR(1) 的 `ρ` 是假设值而非估计值、**水龄在本模型的 168 h 上限内无法收敛**、sensor drift 只测了两个节点上的简化线性斜坡、未建立多传感器随机漂移过程、无任何实测验证）。
