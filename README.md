...
15. **浓度单位曾错 1000 倍，已修正并实测验证不影响任何结论。** WNTR 内部以 kg/m³ 存储浓度，`inpfile_units = "mg/L"` 只管 `.inp` 读写，所以 `initial_quality = 1.0` 实际模拟的是 **1000 mg/L**——进水、水箱、σ、0.2 mg/L 阈值全部高 1000 倍。**为什么一直没暴露**：一阶动力学对浓度线性，且 σ 与阈值同倍错，所有比值精确不变；反应系数不含质量量纲（`k_b` 是 1/time、`k_w` 是 length/time）因而从未受影响，解析已知答案检验一直通过。修正在模型边界完成，并**同步缩放** EPANET 的绝对水质容差（0.01 → 1e−5 kg/m³）——若只改单位不改容差，会引入最大 **17.8%** 相对误差。**实测**：全量 8192 组重跑后报告场相对差 **2.3×10⁻⁷**，日志 1720 个数字只有 2 个变化（解析检验第 6 位小数、优化器迭代计数）。**所有科学结论逐字成立；修正的是浓度的物理解释**——传感器精度、运营阈值、加氯剂量这些工程结论此前建立在错误物理量上，现在才真正成立（Step 15）。
