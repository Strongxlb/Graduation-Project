...
STILL TO CONFIRM BEFORE SUBMISSION (values not derivable from the repository):
  ...
  - The generative-AI statement satisfies both policies now in hand. Imperial Library Services
    ...
    five open the section. The Department of Civil and Environmental Engineering additionally
    requires that AI-generated text, ideas and code be cited as third-party material, and that
    the notice appear FOLLOWING the reference list, which is where it now sits.
  - OPEN QUESTION for the author, and the one that matters most. The departmental policy says
    all submitted work must be "expressed in students' own words, incorporating their own ideas
    and judgements", and that failure to attribute third-party text is plagiarism. The statement
    currently attributes the four-section restructuring to AI. Across the project AI also
    drafted and rewrote a great deal of the prose itself. Decide how much, and say so in the
    statement; nobody else can make that judgement.
    It and the Acknowledgements are now built into paper.docx, between the Conclusions and the
    References, which is where the prescribed layout puts Acknowledgements.
  - Whether those two sections count towards the 12,000 is unresolved and is worth confirming.
    The rule enumerates what counts, the abstract, the body, the conclusions and the figure and
    table captions, and does not name either of them, so they are reported separately rather
    than added to the total. build.sh prints both figures, currently 12,000 without them and
    12,297 with. If the school counts them, 297 words have to come out.
...
I acknowledge the use of Claude Opus 5 (Anthropic, https://claude.ai), accessed through the
Claude Code command-line interface (https://claude.com/claude-code), to restructure the
manuscript, to write the figure and table generation code, and to cross-check the manuscript
against the stored numerical results [@Anthropic2026Claude]. I confirm that no content generated
by AI has been presented as my own work.
