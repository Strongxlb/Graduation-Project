...
STILL TO CONFIRM BEFORE SUBMISSION (values not derivable from the repository):
  ...
  - The generative-AI statement below follows Imperial Library Services' Generative AI guidance,
    which requires the tool name and version, the publisher, the tool URL, a one-sentence
    description of use, and a confirmation that the work is the author's own. All five are in
    the opening paragraph, and the expanded description the guidance permits follows it.
    CHECK the DEPARTMENT's own policy as well: the central guidance says to review it, and it
    may stipulate more. The factual content of the statement is settled either way.
...
against the stored numerical results. I confirm that no content generated by AI has been
presented as my own work.

The paragraphs below give the expanded description. Use ran from 17 May to 12 August 2026 and is
