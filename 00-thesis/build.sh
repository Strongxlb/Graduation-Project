...
# Figures are generated by ../Net3/paper_figs.py and were previously copied here by hand, so a
# rebuild could silently embed a stale panel. Re-copy every time instead.
...
# Excluded sections are not in the built document, so they are not counted and their citation
# keys are not required to resolve. Strip them before anything else looks at the source.
...
# The YAML title block is the title page, not body prose, so it is not counted. Front and back
# matter that is a course requirement rather than argument is split out and reported separately
# below, NOT silently dropped: whether the school counts a declaration towards the 12,000 is
# their call, so both totals are printed.
src = re.sub(r'\A---\n.*?\n---\n', '', src, flags=re.S)
MATTER = ('Statement on the use of generative AI', 'Acknowledgements')
matter = []
for h in MATTER:
    m = re.search(r'^#+\s*%s\s*$(.*?)(?=^#\s|\Z)' % re.escape(h), src, flags=re.M | re.S)
    if m:
        matter.append((h, m.group(1)))
